#include "StoreGraph.h"
    ...
    double StoreGraph::getTotalDistance() {
        return shortestPath.getTotalLength();
    }



    Path StoreGraph::FindPath() {
        Path currentPath;
    ...
    void StoreGraph::pathCall(vector<GraphNode> nodes, GraphNode previousNode, Path currentPath)
    {
        ...
        // Loop though each node and pass the list minus the current node in a recursive call
        for(int i = 0; i < nodes.size(); i++)
        {
            ...
            // Make a copy of the nodes and remove the current node
