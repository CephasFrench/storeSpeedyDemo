...
export default function App() {
  ...
  const [isDeveloper, setIsDeveloper] = useState(
    BOOLEAN_VALUES.SHOW_DEVELOPER_MODE
  );
  ...
  const fetchGroceryList = async () => {
    try {
      const response = await fetch(
        `${SERVER_URL}/grocery_list/${userId}/${location}`
      );
      if (!response.ok) {
  ...
  const handleAddItem = async () => {
    ...
    try {
      ...
      if (data === "YES") {
        ...
        Alert.alert(
          "Item Available",
          `${item} is available at HEB and added to your list.`
        );
      } else if (data === "NO") {
      ...
      Alert.alert(
        "Error",
        `Failed to check item availability: ${error.message}`
      );
    } finally {
      setItem(""); // Clear the text entry box
    }
  ...
  const handleRemoveItem = async (index) => {
    ...
    try {
      const response = await fetch(
        `${SERVER_URL}/update_grocery_list/${userId}/${location}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      Alert.alert(
        "Success",
        `${itemToRemove} has been removed from your grocery list.`
      );
    } catch (error) {
  ...
  const handleGenerateRoute = () => {
    fetch(`${SERVER_URL}/compute_path`)
      ...
      .catch((error) => {
        ...
        Alert.alert(
          "Error",
          `Failed to fetch data from backend: ${error.message}`
        );
      });
  };
