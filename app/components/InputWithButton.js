...
export default function InputWithButton({
  ...
  if (
    ...
    typeof onPressButton !== "function"
  ) {
    console.error(
      "Invalid onChangeText or onPressButton handlers. Expected functions."
    );
    return (
      <Text style={styles.errorText}>
        Failed to load input. Please try again later.
      </Text>
    );
  }
    ...
    BOOLEAN_VALUES.SHOW_ITEM_INPUT && (
      <View style={styles.inputContainer}>
        <TextInput
          ...
          placeholder={label}
        />
        <Button title={buttonLabel} onPress={onPressButton} color="#ff0000" />
      </View>
    )
  );
}
